...
const AgreementForm: React.FC<AgreementFormProps> = ({ onSave, onClose, initialData }) => {
    const { register, handleSubmit, control, formState: { errors }, reset } = useForm<Omit<Agreement, 'id'>>({
        // FIX: Provide explicit generic type to yupResolver to fix type inference issues.
        resolver: yupResolver<Omit<Agreement, 'id'>>(schema),
    });

    useEffect(() => {
        reset(initialData || { name: '', fromDate: '', toDate: '', renewalIntervalDays: null, agreementDate: '' });
    }, [initialData, reset]);

    const onSubmit: SubmitHandler<Omit<Agreement, 'id'>> = (data) => {
        onSave(data);
    };
