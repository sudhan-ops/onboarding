...
import type { Organization, MasterGentsUniforms, SiteGentsUniformConfig, UniformDepartmentConfig, UniformDesignationConfig, GentsPantsSize, GentsShirtSize, SiteStaffDesignation } from '../../types';
...
const UniformTable: React.FC<{
    title: string;
    sizes: (GentsPantsSize | GentsShirtSize)[];
...
}> = ({ title, sizes, headers, control, nestingIndex, quantityType }) => {
    const fits = ['Slim Fit', 'Regular Fit', 'Plump Fit'];
    const sizeGroups = sizes.reduce((acc, size) => {
        acc[size.size] = acc[size.size] || [];
        acc[size.size].push(size);
        return acc;
    }, {} as Record<string, (GentsPantsSize | GentsShirtSize)[]>);
...
const DetailView: React.FC<{
    site: Organization;
    initialConfig: SiteGentsUniformConfig;
    masterUniforms: MasterGentsUniforms;
    siteStaffDesignations: SiteStaffDesignation[];
    onSave: (siteId: string, config: SiteGentsUniformConfig) => Promise<void>;
    onBack: () => void;
}> = ({ site, initialConfig, masterUniforms, siteStaffDesignations, onSave, onBack }) => {
    const [isSaving, setIsSaving] = useState(false);
    const { control, handleSubmit, reset } = useForm<SiteGentsUniformConfig>({ defaultValues: initialConfig });
    ...
    const handleSaveSubmit = async (data: SiteGentsUniformConfig) => {
        setIsSaving(true);
        ...
        <form onSubmit={handleSubmit(handleSaveSubmit)}>
             <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-6">
                <div>
                    ...
                    <h3 className="text-xl font-semibold text-primary-text">Managing Gents' Uniforms for: {site.shortName}</h3>
                </div>
                        ...
                        return (
                            <div key={designation.id} className="p-4 border rounded-lg bg-page/50 space-y-4 relative">
                                ...
                                <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                                    <UniformTable title="Pants" sizes={masterUniforms.pants} headers={[{key:'length',label:'L'},{key:'waist',label:'W'},{key:'hip',label:'H'},{key:'tilesLoose',label:'TL'},{key:'bottomWaist',label:'BW'},{key:'fit',label:'Fit'}]} control={control} nestingIndex={{department: deptIndex, designation: desigIndex}} quantityType="pantsQuantities" />
                                    <UniformTable title="Shirts" sizes={masterUniforms.shirts} headers={[{key:'length',label:'L'},{key:'sleeves',label:'S'},{key:'chest',label:'C'},{key:'shoulder',label:'Sh'},{key:'collar',label:'Co'},{key:'fit',label:'Fit'}]} control={control} nestingIndex={{department: deptIndex, designation: desigIndex}} quantityType="shirtsQuantities" />
                                </div>
                            </div>
                        )
                     })}
...
const ListView: React.FC<{
    sites: Organization[];
    siteConfigs: Record<string, SiteGentsUniformConfig>;
    searchTerm: string;
    setSearchTerm: (term: string) => void;
    onViewDetails: (siteId: string) => void;
}> = ({ sites, siteConfigs, searchTerm, setSearchTerm, onViewDetails }) => {

    const generateSummary = (config?: SiteGentsUniformConfig): string => {
    ...
    return (
        <div>
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-6">
                <h3 className="text-xl font-semibold text-primary-text">Gents' Uniform Site Overview</h3>
...
const UniformChartConfig: React.FC = () => {
    const [view, setView] = useState<'list' | 'detail'>('list');
    const [selectedSiteId, setSelectedSiteId] = useState<string | null>(null);
    
    const [allSites, setAllSites] = useState<Organization[]>([]);
    const [siteConfigs, setSiteConfigs] = useState<Record<string, SiteGentsUniformConfig>>({});
    const [masterUniforms, setMasterUniforms] = useState<MasterGentsUniforms>({ pants: [], shirts: [] });
    ...
    const handleSave = async (siteId: string, config: SiteGentsUniformConfig) => {
        try {
            await api.updateSiteGentsUniforms(siteId, config);
...
export default UniformChartConfig;