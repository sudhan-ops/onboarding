...
import type { ManpowerDetail } from '../../types';
...
const ManpowerDetailsModal: React.FC<ManpowerDetailsModalProps> = ({
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    const nextElement = formContainer.querySelector(`input[data-row-index="${nextRowIndex}"][data-col-index="${colIndex}"]`) as HTMLElement;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 p-4" onClick={onClose}>
      <div className="bg-card rounded-xl shadow-card p-6 w-full max-w-3xl m-4 animate-fade-in-scale flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            {isLoading ? (
              ...
              <div className="space-y-2">
                <div className="grid grid-cols-12 gap-4 sticky top-0 bg-page p-2 rounded-t-lg z-10">
                    <div className="col-span-8 font-medium text-sm text-muted">Designation</div>
                    <div className="col-span-3 font-medium text-sm text-muted">Count</div>
