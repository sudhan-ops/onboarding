...
const TaskForm: React.FC<TaskFormProps> = ({ isOpen, onClose, initialData, setToast }) => {
  ...
  const { register, handleSubmit, formState: { errors }, reset, control, watch } = useForm<TaskFormInputs>({
    ...
    resolver: yupResolver<TaskFormInputs>(validationSchema),
  });
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                className="mt-1 bg-card border border-border rounded-lg px-3 py-2.5 w-full sm:text-sm focus:ring-1 focus:ring-accent"
              />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Controller
                ...
                control={control}
                render={({ field }) => (
                  <DatePicker label="Due Date (Optional)" id="dueDate" value={field.value} onChange={field.onChange} />
                )}
              />
             ...
             <Select label="Assign To (Optional)" id="assignedToId" registration={register('assignedToId')} error={errors.assignedToId?.message}>
