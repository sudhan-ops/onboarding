...
const SlideToConfirm: React.FC<SlideToConfirmProps> = ({
  ...
  }, [isSubmitting, isConfirmed, snapBack]);
  
  const isActionInProgress = isSubmitting || isConfirmed;
  const showSuccess = isConfirmed && !isSubmitting;
  ...
  return (
    ...
    >
      ...
      <span className={`fo-slider-text ${position !== 0 || isActionInProgress ? 'opacity-0' : 'opacity-100'}`}>{text}</span>
      <span className={`fo-slider-text absolute transition-opacity duration-300 ${isActionInProgress ? 'opacity-100' : 'opacity-0'}`}>{confirmText}</span>
    </div>
  );
};
