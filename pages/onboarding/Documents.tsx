...
const Documents: React.FC = () => {
    ...
    // FIX: Add missing 'updateGmc' to store destructuring.
        ...
        return (
            <form onSubmit={async (e) => { e.preventDefault(); await onValidated(); }} id="documents-form">
                ...
                <div className="space-y-4">
                    <UploadDocument label="ID Proof (Front Side)" file={data.personal.idProofFront} onFileChange={(file) => updatePersonal({ idProofFront: file })} onOcrComplete={handlePersonalIdOcr} ocrSchema={idProofSchema} setToast={setToast} allowCapture />
                    <UploadDocument label="ID Proof (Back Side)" file={data.personal.idProofBack} onFileChange={(file) => updatePersonal({ idProofBack: file })} onOcrComplete={handlePersonalIdOcr} ocrSchema={idProofSchema} setToast={setToast} allowCapture />
                    <UploadDocument label="Profile Photo" file={data.personal.photo} onFileChange={(file) => updatePersonal({ photo: file })} allowCapture />
                    <UploadDocument label="Bank Proof" file={data.bank.bankProof} onFileChange={(file) => updateBank({ bankProof: file })} onOcrComplete={handleBankOcr} ocrSchema={bankProofSchema} setToast={setToast} allowCapture />
                    <UploadDocument label="Latest Salary Slip (Optional, for UAN/ESI)" file={data.uan.salarySlip} onFileChange={(file) => updateUan({ salarySlip: file })} onOcrComplete={(d) => { handleUanOcr(d); handleEsiOcr(d); }} ocrSchema={salarySlipSchema} setToast={setToast} allowCapture />
                    {data.uan.hasPreviousPf && <UploadDocument label="UAN Document" file={data.uan.document} onFileChange={(file) => updateUan({ document: file })} onOcrComplete={handleUanOcr} ocrSchema={uanSchema} setToast={setToast} allowCapture />}
                    ...
                    <div className="mt-6 pt-6 border-t border-[#374151]">
                        <h4 className="fo-section-title mb-4">Education Certificates</h4>
                        <div className="space-y-4">
                            ...
                            <button type="button" onClick={() => navigate('/onboarding/add/education')} className="fo-btn-add-item">
                    ...
                    <div className="mt-6 pt-6 border-t border-[#374151]">
                        <h4 className="fo-section-title mb-4">Family Member Documents</h4>
                         <div className="space-y-4">
                            {data.family.map(member => (
                                <UploadDocument key={member.id} label={`ID Proof for ${member.name || 'Family Member'}`} file={member.idProof} onFileChange={(file) => updateFamilyMember(member.id, { idProof: file })} onOcrComplete={handleFamilyOcr(member.id)} ocrSchema={idProofSchema} setToast={setToast} allowCapture />
                            ))}
                            {data.family.length === 0 && <p className="text-sm text-gray-400">No family members added.</p>}
                            <button type="button" onClick={() => navigate('/onboarding/add/family')} className="fo-btn-add-item">
                                <Plus className="mr-2 h-4 w-4" />
                ...
                <section>
                    ...
                    <div className="space-y-4">
                        ...
                        <Button type="button" variant="outline" onClick={() => navigate('/onboarding/add/family')}>
                            <Plus className="mr-2 h-4 w-4" /> Add / Edit Family Members
                        </Button>
                    </div>
                </section>
            </div>
