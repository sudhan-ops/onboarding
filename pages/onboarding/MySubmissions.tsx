...
import { Loader2, Plus, Search, ArrowLeft, UserPlus, CheckSquare, Square } from 'lucide-react';
...
const SubmissionCard: React.FC<{ submission: OnboardingData }> = ({ submission }) => {
    ...
    const progress = useMemo(() => {
        ...
        if (isEligibleForEsi) {
            ...
            if (gmc.isOptedIn === true) {
                // FIX: Corrected a typo from `gmcComplete` to `esiGmcComplete` to resolve a "Cannot find name" error.
    ...
    return (
        <Link to={`/onboarding/add/personal?id=${submission.id}`} className="block">
            <div className="bg-[#243524] p-3 rounded-xl flex gap-4 items-start border border-transparent hover:border-emerald-500 transition-colors duration-200">
                ...
                <div className="flex-grow">
                     <div className="flex justify-between items-start mb-1">
                        <p className="font-semibold text-white pr-2">{`${submission.personal.firstName} ${submission.personal.lastName}`}</p>
        ...
        <div className="h-full flex flex-col bg-[#0f1f0f] text-white">
            ...
            <header className="p-4 flex-shrink-0 flex items-center justify-between fo-mobile-header sticky top-0 z-10 bg-[#0f1f0f]/80 backdrop-blur-sm border-b border-[#374151]">
                ...
                <div className="flex items-center gap-4">
                    <button onClick={handleSearchIconClick} aria-label="Search employee">
                        <Search className="h-6 w-6" />
                    </button>
                    <button onClick={handleAddNew} aria-label="Add new employee">
            ...
            <div className="px-4 py-3 flex-shrink-0">
                <div className="flex border-b-2 border-[#374151]">
                    {filterTabs.map(tab => (
                        <button
                            key={tab.key}
                            onClick={() => setStatusFilter(tab.key)}
                            className={`flex-1 py-2 text-sm font-semibold uppercase tracking-wider transition-colors duration-200 ${
            ...
            <main className="flex-1 overflow-y-auto px-4 space-y-3 pb-24">
                <div className="relative">
                    ...
                    <input
                        ...
                        className="fo-input !pl-10 !py-2 !rounded-md"
                    />
                </div>
