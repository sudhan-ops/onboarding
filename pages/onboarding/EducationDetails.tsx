...
const EducationDetails: React.FC = () => {
    ...
    const { control, handleSubmit, formState: { errors }, setValue, reset } = useForm<{ education: EducationRecord[] }>({
        resolver: yupResolver(educationDetailsSchema),
        defaultValues: { education: onboardingData.education },
                ...
                <div className="space-y-4">
                    {fields.map((field, index) => (
                        <div key={field.id} className="p-4 border border-border rounded-xl relative space-y-4">
                            ...
                            <Controller name={`education.${index}.degree`} control={control} render={({ field }) => <input placeholder="Degree / Certificate" {...field} className="fo-input"/>} />
                            <Controller name={`education.${index}.institution`} control={control} render={({ field }) => <input placeholder="Institution / University" {...field} className="fo-input"/>} />
                            <Controller name={`education.${index}.endYear`} control={control} render={({ field }) => <input placeholder="Year of Completion" type="text" maxLength={4} {...field} className="fo-input"/>} />
                 ...
                 <button type="button" onClick={handleAddRecord} className="mt-6 w-full fo-btn-secondary flex items-center justify-center">
